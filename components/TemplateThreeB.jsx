...
const filterDB = ['casas', 'alquileres', 'anticreticos', 'lotes', 'casas', 'alquileres', 'anticreticos', 'lotes']
...
function TemplateFour({ color, topic, post1, post2, post3, post4, description1, description2, description3, description4,
    ...
    return (
        <section className={styles.section} id={topic} style={{ backgroundColor: color }}>
            {topic != "Inicio" && <div className={styles.containerSubtitle}><h4 className={styles.subtitle}>{topic == 'Servicios' ? 'SERVICIOS' : topic.toUpperCase()}</h4></div>}

            {userDB[topic]["BannerTop"] && <Banner ruta={topic} carpeta="BannerTop" click={handlerClickEnlace}></Banner>}

            {topic != "Inicio" &&<div> <button className={styles.buttonSeeAll} onClick={()=>setFilter(!filter)}>filtrar</button> <button className={styles.buttonSeeAll} onClick={setPostsElements}>Ver todo</button></div>}

            {topic != "Inicio" && <div className={`${styles.gridFilter} `}>
            {filterDB.map((i)=><Tag>{i}</Tag>)}
            </div>}
